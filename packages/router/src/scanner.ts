...
export interface ScanRoutesOptions {
  ...
  /**
   * Hint: absolute path of the single file that changed. When provided and
   * a cache hit exists, the scanner fast-patches only that file's entry
   * instead of re-walking the entire directory tree.
   */
  changedFile?: string;
}
 ...
 */
function detectComponentType(filePath: string, knownSignature?: string): "server" | "client" {
