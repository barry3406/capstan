...
export function analyzeRouteFileStaticInfo(
  ...
  const hasAction = exportNames.includes("action");
  ...
  const staticInfo = exportNames.length > 0 || renderMode || revalidate !== undefined || hasMetadata || hasAction || hasGenerateStaticParams
    ? {
        exportNames,
