 ...
 *
 * React 19 metadata hoisting: `<title>`, `<meta>`, and `<link>` elements
 * rendered anywhere in the component tree are automatically hoisted to
 * `<head>` by React 19's built-in document metadata support. This means
 * these elements can be returned from any component — not just from a
 * top-level `<head>` wrapper — and React will place them correctly.
 *
 * @see https://react.dev/blog/2024/12/05/react-19#support-for-document-metadata
 */
