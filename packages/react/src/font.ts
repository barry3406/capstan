...
/**
 * Preload a font using React 19's built-in resource preloading API.
 *
 * React 19 provides `preload()` from `react-dom` which integrates with
 * the rendering pipeline to emit `<link rel="preload">` tags at the
 * optimal time during SSR streaming, avoiding manual `<link>` element
 * construction.
 *
