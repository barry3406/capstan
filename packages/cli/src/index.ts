...
async function runAnalyze(args: string[]): Promise<void> {
  ...
  const checkBudgetFlag = hasFlag(args, "--budget");
  const budgetFile = readFlagValue(args, "--budget-file");
  const buildDirArg = readFlagValue(args, "--build-dir");

  const cwd = process.cwd();
  const buildDir = buildDirArg
    ? resolve(cwd, buildDirArg)
    : join(cwd, "dist", "client");

  if (!existsSync(buildDir)) {
    console.error(pc.red(`Build directory not found: ${buildDir}`));
    console.error(pc.dim("  Run `capstan build` first to generate build output."));
    process.exitCode = 1;
    return;
  }

  const devMod = (await import("@zauso-ai/capstan-dev")) as unknown as {
    analyzeBundle: (opts: {
      buildDir: string;
      manifestPath?: string | undefined;
      routeManifest?: { routes: Array<{ urlPattern: string; filePath: string; type: string }> } | undefined;
    }) => Promise<{
      timestamp: string;
      totalSize: { raw: number; gzip: number; brotli: number };
      jsSize: { raw: number; gzip: number; brotli: number };
      cssSize: { raw: number; gzip: number; brotli: number };
      routes: Array<{
        pattern: string;
        filePath: string;
        js: { raw: number; gzip: number; brotli: number };
        css: { raw: number; gzip: number; brotli: number };
        total: { raw: number; gzip: number; brotli: number };
        chunks: string[];
      }>;
      chunks: Array<{
        name: string;
        size: { raw: number; gzip: number; brotli: number };
        modules: string[];
        isEntry: boolean;
        isDynamic: boolean;
      }>;
    ...
    checkBudgets: (analysis: Record<string, unknown>, budgets: Record<string, unknown>) => { passed: boolean; violations: Array<{ rule: string; limit: number; actual: number; target?: string; message: string }> };
    formatAnalysisTable: (analysis: Record<string, unknown>) => string;
    formatAnalysisSummary: (analysis: Record<string, unknown>) => string;
    formatBudgetReport: (result: Record<string, unknown>) => string;
    DEFAULT_BUDGETS: Record<string, unknown>;
  };
  const {
    analyzeBundle,
    checkBudgets,
  ...
  const routeManifestPath = join(cwd, "dist", "_capstan_manifest.json");
  let routeManifest: { routes: Array<{ urlPattern: string; filePath: string; type: string }> } | undefined;
  if (existsSync(routeManifestPath)) {
    try {
      const raw = await readFile(routeManifestPath, "utf-8");
      routeManifest = JSON.parse(raw) as typeof routeManifest;
    } catch {
      // Ignore parse errors
    }
  }
  ...
  const viteManifestPath = join(buildDir, ".vite", "manifest.json");
  const manifestPath = existsSync(viteManifestPath) ? viteManifestPath : undefined;
  ...
  if (asJson && !checkBudgetFlag) {
    console.log(JSON.stringify(analysis, null, 2));
    return;
  }

  if (!asJson) {
    process.stdout.write(formatAnalysisTable(analysis));
    process.stdout.write("\n");
    process.stdout.write(formatAnalysisSummary(analysis));
  }
  ...
  if (checkBudgetFlag) {
    ...
    if (budgetFile) {
      const budgetPath = resolve(cwd, budgetFile);
      try {
        const raw = await readFile(budgetPath, "utf-8");
        budgets = JSON.parse(raw) as typeof budgets;
      } catch (err: unknown) {
        console.error(pc.red(`Failed to read budget file: ${budgetPath}`));
        process.exitCode = 1;
        return;
      }
    }
    ...
    if (asJson) {
      console.log(JSON.stringify({ analysis, budgetResult: result }, null, 2));
    } else {
      process.stdout.write("\n");
...
function printHelp(): void {
  ...
  group("Verification", [
    ...
    ["analyze [--json] [--budget] [--build-dir <path>]", "Analyze client bundle sizes and budgets"],
  ]);
