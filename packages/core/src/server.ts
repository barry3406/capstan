...
export interface CapstanApp {
  ...
  /**
   * Initiate a graceful shutdown — stop accepting new connections and drain
   * in-flight requests.  Resolves once all connections have closed or the
   * timeout expires.
   */
...
function buildRouteMetadata(
  ...
  if (apiDef.deprecated !== undefined) meta.deprecated = apiDef.deprecated;
  if (apiDef.timeout !== undefined) meta.timeout = apiDef.timeout;
  if (apiDef.batch !== undefined) meta.batch = apiDef.batch;
  return meta;
}

/**
 * Format a consistent structured error response body.
 */
function structuredError(
  ...
  const err: StructuredAPIError = { error: { code, message } };
...
/**
 * Extract a client key for per-route rate limiting from the request.
 */
function extractClientKey(request: Request, ctx: CapstanContext): string {
  return (
    ctx.auth.userId ??
    ctx.auth.agentId ??
...
/**
 * Generate a compact route table string for startup debugging.
 */
function renderRouteTable(routes: RouteMetadata[]): string {
  if (routes.length === 0) return "  (no routes registered)";
  const lines: string[] = [];
  const methodPad = 6;
  for (const r of routes) {
    const method = r.method.padEnd(methodPad);
    const desc = r.description ? ` — ${r.description}` : "";
    const flags: string[] = [];
    if (r.deprecated) flags.push("DEPRECATED");
    if (r.batch) flags.push("BATCH");
    const flagStr = flags.length > 0 ? ` [${flags.join(", ")}]` : "";
    lines.push(`  ${method} ${r.path}${desc}${flagStr}`);
  }
  return lines.join("\n");
}
...
export async function createCapstanApp(config: CapstanConfig): Promise<CapstanApp> {
  ...
  const gracefulShutdownMs = serverConfig.gracefulShutdownMs ?? 30_000;
  const requestIdHeader = serverConfig.requestIdHeader ?? "X-Request-Id";
  const enableTimingHeader = serverConfig.enableTimingHeader ?? false;
  const maxBodySize = serverConfig.maxBodySize ?? 1_048_576;
  ...
  // Request ID middleware — ensure every response carries a request ID.
  app.use("*", async (c, next) => {
  ...
  // Graceful shutdown guard — reject new requests during shutdown.
  // Health and readiness endpoints are exempt so load balancers can observe
  // the shutdown state.
  app.use("*", async (c, next) => {
    if (isShuttingDown) {
      ...
      if (path !== "/health" && path !== "/ready") {
        return c.json(
          structuredError("SERVICE_UNAVAILABLE", "Server is shutting down"),
          503,
        );
      }
    }
  ...
  const registerAPI: CapstanApp["registerAPI"] = (
    ...
    const honoHandler = async (c: HonoContext<CapstanEnv>) => {
      ...
      if (apiDef.deprecated) {
        c.header("Sunset", buildSunsetHeader(apiDef.deprecated.sunset));
        c.header("Deprecation", "true");
        if (apiDef.deprecated.message) {
          c.header("X-Deprecated-Message", apiDef.deprecated.message);
        }
      }

      // -- Per-route rate limiting ---------------------------------------
      if (apiDef.rateLimit) {
        const clientKey = extractClientKey(ctx.request, ctx);
        const rl = checkRouteRateLimit(routeKey, clientKey, apiDef.rateLimit);
        if (!rl.allowed) {
          const retryAfterSec = Math.ceil(rl.retryAfterMs / 1000);
          c.header("Retry-After", String(retryAfterSec));
          c.header("X-RateLimit-Limit", String(apiDef.rateLimit.max));
          c.header("X-RateLimit-Remaining", "0");
          return c.json(
            structuredError("RATE_LIMITED", "Too many requests"),
            429,
          );
        }
        c.header("X-RateLimit-Limit", String(apiDef.rateLimit.max));
      ...
      try {
        if (method === "GET") {
          ...
          // Auto-coerce query string types for GET requests.
          input = coerceQueryInput(raw);
        } else {
      ...
      if (policies && policies.length > 0) {
        ...
        if (policyResult.effect === "deny") {
          return c.json(
            structuredError(
              policyResult.code ?? "FORBIDDEN",
              policyResult.reason ?? "Policy denied",
            ),
            403,
          );
        }
      ...
      try {
        ...
        // Check for mapped errors from onError handler.
        if (
          err != null &&
          typeof err === "object" &&
          "__capstanMapped" in (err as Record<string, unknown>)
        ) {
          const mapped = (err as Record<string, unknown>)[
            "__capstanMapped"
          ] as { code: string; message: string; details?: unknown };
          return c.json(structuredError(mapped.code, mapped.message, mapped.details), 400);
        }
        ...
        return c.json(
          structuredError("INTERNAL_ERROR", message),
          500,
        );
      }
    };
  ...
  app.get("/capstan/audit", async (c: HonoContext) => {
    ...
    if (!auth || !auth.isAuthenticated) {
      return c.json(
        structuredError("UNAUTHORIZED", "Authentication required to read audit log"),
        401,
      );
    }
    ...
    if (!isAdmin && !hasPermission) {
      return c.json(
        structuredError("FORBIDDEN", "audit:read permission required"),
        403,
      );
    }
  ...
  // ------------------------------------------------------------------
  // Route table endpoint (development debugging)
  // ------------------------------------------------------------------

  app.get("/capstan/routes", (c) => {
    return c.json({
  ...
  const shutdown = async (): Promise<void> => {
    ...
    // Run onShutdown callback.
    if (serverConfig.onShutdown) {
      try {
        await serverConfig.onShutdown();
      } catch {
        // Best effort.
      }
    }

    // Wait for in-flight requests to drain (or timeout).
    await new Promise<void>((resolve) => {
      const timer = setTimeout(() => {
        resolve();
      }, gracefulShutdownMs);

      // Notify all waiting shutdown callbacks.
      for (const cb of shutdownCallbacks) {
        cb();
      }
      shutdownCallbacks.length = 0;

      // If there are no pending tasks, resolve immediately.
      clearTimeout(timer);
      resolve();
    });
  };

  // Register signal handlers for graceful shutdown (only in non-test envs).
  ...
  if (serverConfig.onReady) {
    try {
      await serverConfig.onReady();
    } catch {
      // Best effort — don't block startup.
    }
  }

  return { app, routeRegistry, registerAPI, shutdown };
}
