...
import type { AgentCheckpoint, MemoryScope } from "../../types.js";
import { assertValidAgentCheckpoint, assertValidCheckpointRecord } from "./checkpoint.js";
import {
...
export class FileHarnessRuntimeStore implements HarnessRuntimeStore {
  ...
  async persistCheckpoint(runId: string, checkpoint: AgentCheckpoint): Promise<HarnessRunCheckpointRecord> {
    await this.initialize();
    ...
    const record: HarnessRunCheckpointRecord = {
      ...
      updatedAt: new Date().toISOString(),
      checkpoint: toPersistableValue(checkpoint) as AgentCheckpoint,
    };
    await writeJsonAtomic(
