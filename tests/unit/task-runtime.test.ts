...
import { createSmartAgent } from "../../packages/ai/src/smart-agent.ts";
...
describe("task helpers", () => {
  ...
  it("supports tool helpers inside the agent loop itself", async () => {
    const llm = mockLLM([
    ...
    const result = await agent.run("run remote task");
