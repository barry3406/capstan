...
export const DOC_SECTIONS: DocSection[] = [
  ...
  {
    slug: "smart-agent-runtime",
    title: "Smart Agent Runtime",
    category: "Guide",
    url: "/docs/smart-agent-runtime",
    topics: ["createSmartAgent", "defineSkill", "evolution", "skills", "agent", "compression", "fallback", "validation", "timeout", "watchdog", "budget"],
    summary: "Build production agents with createSmartAgent(). 12 built-in features: compression, fallback, validation, timeouts, watchdog, budgets. defineSkill() for strategic guidance. Self-evolution that learns from every run.",
  },
  {
    slug: "database",
    title: "Database",
  ...
  },
  {
    ...
    category: "Reference",
    url: "/docs/comparison",
    topics: ["comparison", "nextjs", "langchain", "fastapi", "architecture", "zero-wall"],
    summary: "Detailed comparison of Capstan vs Next.js vs LangChain across dimensions including multi-protocol, agent support, skill layer, evolution, auth, database, AI toolkit, and the zero-wall architecture.",
  },
];
...
export const DOC_PAGES: DocPage[] = [
  ...
  },
  {
    ...
    section: "Getting Started",
    url: "/docs/getting-started",
    content: "app/routes/ for file-based routing. app/models/ for database models (defineModel). app/styles/ for CSS entry points. app/policies/ for permission policies. app/migrations/ for database migrations. app/public/ for static files served from root. agents/ for smart agent definitions. capstan.config.ts for framework configuration. AGENTS.md for AI agent guide.",
    keywords: ["structure", "directory", "files", "routes", "models", "config", "agents"],
  ...
  {
    slug: "smart-agent-runtime",
    title: "createSmartAgent()",
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "createSmartAgent() creates a production-ready agent with 12 built-in features. Configuration: model (LLM model to use), tools (array of tool definitions), skills (array of defineSkill() results), maxTurns (max agent loop iterations), timeout ({ total, perTool } milliseconds), budget ({ maxTokens } token budget), compression ({ strategy } context compression), fallback ({ model } fallback model on failure), validation ({ validateToolArgs } Zod validation of tool arguments), watchdog ({ maxConsecutiveErrors } error circuit breaker), persistDir (directory for durable state), bufferTokens (token buffer for context window).",
    keywords: ["createSmartAgent", "agent", "smart-agent", "production", "compression", "fallback", "validation", "timeout", "watchdog", "budget", "persistDir", "bufferTokens"],
    topics: ["agent", "smart-agent-runtime"],
  },
  {
    slug: "smart-agent-runtime",
    title: "defineSkill()",
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "defineSkill() gives agents high-level strategic guidance beyond individual tools. A skill has: name (identifier), when (condition string describing when to apply), strategy (instruction string for the agent's approach). Skills are not tools — they provide strategic patterns for solving complex problems. Example: defineSkill({ name: 'triage', when: 'ticket needs priority assessment', strategy: 'Check severity, customer tier, SLA deadline' }).",
    keywords: ["defineSkill", "skill", "strategy", "guidance", "pattern", "when", "approach"],
    topics: ["agent", "skill", "smart-agent-runtime"],
  },
  {
    slug: "smart-agent-runtime",
    ...
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "Agents learn from every run through the evolution pipeline: Experience (raw execution trace) -> Strategy (distilled patterns) -> Skill (reusable defineSkill). The evolution engine tracks what worked and what failed, distilling successful patterns into new skills that improve future runs. This creates a virtuous cycle where agents get smarter over time without manual tuning.",
    keywords: ["evolution", "self-evolution", "learning", "experience", "strategy", "distill", "improve", "virtuous-cycle"],
    topics: ["agent", "evolution", "smart-agent-runtime"],
  },
  {
    slug: "smart-agent-runtime",
    ...
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "Smart agents support context compression to handle long conversations within token limits. Strategies: 'sliding-window' (keep recent messages, summarize old), 'summarize' (compress entire history into summary). Configured via compression: { strategy: 'sliding-window' } in createSmartAgent(). Works with bufferTokens to maintain headroom in the context window.",
    keywords: ["compression", "sliding-window", "summarize", "context", "tokens", "buffer"],
    topics: ["agent", "compression", "smart-agent-runtime"],
  },
  {
    slug: "smart-agent-runtime",
    title: "Agent Fallback and Watchdog",
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "Fallback: when the primary model fails, automatically retry with a fallback model. Configured via fallback: { model: 'claude-haiku-4' }. Watchdog: circuit breaker that stops agent loops after maxConsecutiveErrors consecutive tool failures, preventing infinite error loops. Configured via watchdog: { maxConsecutiveErrors: 3 }.",
    keywords: ["fallback", "watchdog", "circuit-breaker", "error", "retry", "recovery"],
    topics: ["agent", "reliability", "smart-agent-runtime"],
  },
  {
    slug: "smart-agent-runtime",
    ...
    section: "Smart Agent Runtime",
    url: "/docs/smart-agent-runtime",
    content: "When validation.validateToolArgs is true, tool arguments are validated against their Zod schemas before execution. Invalid arguments are caught early with clear error messages sent back to the LLM, preventing downstream failures. This is especially important for production agents where tool calls interact with external systems.",
    keywords: ["validation", "validateToolArgs", "zod", "schema", "tool-args", "type-safety"],
    topics: ["agent", "validation", "smart-agent-runtime"],
  },
  ...
  // AI Toolkit / Smart Agent Runtime
  {
    slug: "core-concepts",
    title: "Smart Agent Runtime",
    section: "Core Concepts",
    url: "/docs/core-concepts",
    content: "@zauso-ai/capstan-ai provides the smart agent runtime. createSmartAgent() for production agents with 12 built-in features. defineSkill() for strategic guidance beyond tools. Self-evolution pipeline: Experience -> Strategy -> Skill. Also includes think(llm, prompt) for single LLM calls, generate() for streaming, remember(fact) and recall(query) for persistent memory, and createHarness() for durable agent runtime with checkpoints, approvals, and recovery.",
    keywords: ["ai", "toolkit", "createSmartAgent", "defineSkill", "evolution", "think", "generate", "remember", "recall", "harness", "llm", "smart-agent"],
    topics: ["ai", "ai-toolkit", "smart-agent-runtime"],
  },
  ...
  },
  {
    ...
    section: "API Reference",
    url: "/docs/api-reference",
    content: "createSmartAgent({ model, tools, skills, maxTurns, timeout, budget, compression, fallback, validation, watchdog, persistDir, bufferTokens }): Create production-ready agent. defineSkill({ name, when, strategy }): Define strategic skill for agents. createAI({ llm }): Initialize AI toolkit. think(llm, prompt, options): Single LLM call with optional Zod schema for structured output. generate(llm, prompt): Streaming LLM call. thinkStream(llm, prompt): Streaming with structured output. remember(fact): Store to long-term memory. recall(query): Retrieve from memory. createHarness({ appName, runtimeDir }): Create durable agent runtime.",
    keywords: ["ai", "createSmartAgent", "defineSkill", "createAI", "think", "generate", "thinkStream", "remember", "recall", "createHarness", "evolution", "skills"],
    topics: ["api", "reference", "ai", "smart-agent-runtime"],
  },
  {
    slug: "api-reference",
...
export const POST = defineAPI({
  ...
  {
    title: "Smart Agent with Skills",
    description: "Create a production-ready smart agent with skills, compression, fallback, and watchdog",
    code: `import { createSmartAgent, defineSkill } from "@zauso-ai/capstan-ai";

const triageSkill = defineSkill({
  name: "triage",
  when: "ticket needs priority assessment",
  strategy: "Check severity, customer tier, SLA deadline. Assign P1 for production-down, P2 for degraded, P3 for cosmetic.",
});
...
const result = await agent.run("Triage all open P0 tickets");`,
    language: "typescript",
    topics: ["createSmartAgent", "defineSkill", "agent", "skill", "smart-agent-runtime"],
    relatedDocs: "/docs/smart-agent-runtime",
  },
  {
    title: "Skill Definition",
    description: "Define reusable strategic skills for agents with defineSkill()",
    code: `import { defineSkill } from "@zauso-ai/capstan-ai";

// Skills provide strategic guidance, not just tool access
const customerSupport = defineSkill({
  name: "customer-support",
  when: "user has a support question or issue",
  strategy: "Search docs first, then check ticket history, escalate to human if unresolved after 3 attempts.",
});

const escalation = defineSkill({
  name: "escalation",
  when: "issue is critical or customer is enterprise tier",
  strategy: "Immediately page on-call, create P1 ticket, notify account manager, then investigate root cause.",
});

const dataAnalysis = defineSkill({
  name: "data-analysis",
  when: "user needs insights from data",
  strategy: "Clarify the question, query relevant tables, visualize trends, highlight anomalies.",
});`,
    language: "typescript",
    topics: ["defineSkill", "skill", "strategy", "agent", "smart-agent-runtime"],
    relatedDocs: "/docs/smart-agent-runtime",
  },
  {
